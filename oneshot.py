...
class WiFiScanner:
    ...
    def prompt_network(self) -> str:
        ...
        if i:  # User pressed Enter
            input_value = sys.stdin.readline().strip()
            try:
                if input_value.lower() in ('r', '0', ''):
                    return self.prompt_network()
                elif int(input_value) in networks.keys():
                    return networks[int(input_value)]['BSSID']
                else:
                    raise ValueError
            except ValueError:
                print('Invalid number')
            ...
            print("[*] Auto-refreshing network list...")
            return self.prompt_network()
